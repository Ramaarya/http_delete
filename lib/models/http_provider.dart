...
class HttpProvider with ChangeNotifier {
  ...
  void connectAPI(String id, BuildContext context) async {
    ...
    if (hasilResponse.statusCode == 200) {
      ...
      notifyListeners();
      handlingData(context, 'Berhasil mendapat data');
    } else {
      // gagal mengambil data
      handlingData(context, 'Error ${hasilResponse.statusCode}');
    }
  }
  ...
  void deleteData(BuildContext context) async {
    ...
    if (hasilResponse.statusCode == 204) {
      _data = {};
      notifyListeners();
      handlingData(context, 'No Content !');
    }
  }

  handlingData(BuildContext context, String message) {
